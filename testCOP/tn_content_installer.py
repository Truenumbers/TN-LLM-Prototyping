...
try:
    ...
    if tnTriggerBase == "":
        tnTriggerBase = 'http://gov.truenumbers.com/truenumbers-trigger-api'
...
try:
    ...
    if tnArtifactBase == "":
         tnArtifactBase = 'http://gov.truenumbers.com/truenumbers-artifact-api'
...
try:
    ...
    if tnRestBase == "":
         tnRestBase = 'http://gov.truenumbers.com/truenumbers-rest-api'
...
# All dirs in the root dir of the install are expected to be numberspaces, unless first char is underscore

archiveList = [x for x in dList[0] if  not x.startswith("_")] ## List if nspace archive dirs
...
# Ask user about deleting existing numberspace before making a new one
    ...
    if nsDirName not in cleanNsList:
        ...
        keyPress = input("Numberspace " + nsDirName + " exists, clear its content and triggers? (Y/N): ")
